...
module.exports = {
  ...
  editarConfiguracion: async (req, res) => {
    try {
      const { IdParametro, Nombre, Valor, Slug, Descripcion, CHUSER } =
        req.body;

      const result = await utils.executeQuery(
        "CALL sp_EditarParametro(?,?,?,?,?,?)",
  ...
  getParametro: async (req, res) => {
    try {
      console.log(req.body);
      const { Nombre } = req.body;
      const result = await utils.executeQuery("CALL sp_ObtenerParametro(?)", [
        Nombre,
      ]);
